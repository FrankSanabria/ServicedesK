...
for ($i=0;$i<$faq_count1;$i++) {
	...
	$v_answere = trim(substr($v_answere, 0, 200));
	$v_answere .=".............";

	if (ON == plugin_config_get('faq_view_window') ){
		if( helper_get_current_project() == '0000000' ){
			PRINT "<div><span class=\"faq-question\"><a href=\"$g_faq_view_page&f_id=$v_id\" target=_new>$v_question</a> [$t_project_name] </span><br><span>$v_answere</span></div><br>";
		}else{
			PRINT "<div><span class=\"faq-question\"><a href=\"$g_faq_view_page&f_id=$v_id\" target=_new>$v_question</a></span><br><span>$v_answere</span></div><br>";
		}
	} else{
		if( helper_get_current_project() == '0000000' ){
			PRINT "<div><span class=\"faq-question\"><a href=\"$g_faq_view_page&f_id=$v_id\" >$v_question</a> [$t_project_name] </span><br><span>$v_answere</span></div><br>";
		}else{
			PRINT "<div><span class=\"faq-question\"><a href=\"$g_faq_view_page&f_id=$v_id\" >$v_question</a></span><br><span>$v_answere</span></div><br>";
		}
	}
    PRINT "<span  class=\"small\">$v_date_posted - <a class=\"faq-email\" href=\"mailto:$t_poster_email\">$t_poster_name</a></span><br>";
