	...
	public static function display($location, $project_id=null, $css_class="") {
		if (auth_is_user_authenticated()) {
			...
			if ($message !== null) {
				...
				$tittle = "<h2 style=\"text-align:center;\">{$message->title}</h2>";
                                $html2 = "<span class=\"message\">";


				if ($context->dismissable) {
	                               	$image = plugin_file("dismiss.png");
					$html = "<img class=\"announcement-dismiss\" src=\"{$image}\" alt=\"Dismiss Announcement\" value=\"{$context->id}\"/>{$html2}";
				} else {
	                                $html = $html2;
				}
				echo "<div class=\"announcement {$css_class}\">" . $html . $tittle . $message->message . "</span></div>";
