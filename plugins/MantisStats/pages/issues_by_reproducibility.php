...
$reproducibility_enum_string    = lang_get( 'reproducibility_enum_string' );
...
$query = "
        SELECT count(*) as the_count, reproducibility, status
        FROM $mantis_bug_table
        WHERE $specific_where
        AND date_submitted >= " . db_prepare_string( $db_datetimes['start'] ) . "
        AND date_submitted <= " . db_prepare_string( $db_datetimes['finish'] ) . "
        GROUP BY reproducibility, status
        ";
$result = db_query( $query );

foreach ( $result as $row ) {
    $issues_fetch_from_db[$row['reproducibility']][$row['status']] = $row['the_count'];
}
...
function tables_and_charts ( $type, $output ) {
	...
	global $resolved_status_threshold, $status_values, $status_enum_string, $reproducibility_enum_string, $issues_fetch_from_db, $maxResultsInTables;
    ...
    <table class='display' id='$type' style='display:none'>
        <thead>
        <tr class='tblheader'>
            <td width='100%'>" . lang_get( 'reproducibility' ) . "</td>";
    ...
    foreach ( $data_table_totals as $key => $val ) {
		...
		$data_table_print .= "
        <tr>
            <td>" . MantisEnum::getLabel( $reproducibility_enum_string, $key ) . "</td>";
...
EOT;
    ...
    foreach ( $data_table_totals as $key => $val ) {
        ...
        $chart_data_print .= "<set label='" . MantisEnum::getLabel( $reproducibility_enum_string, $key ) . "' value='" . $val . "' toolText='" . number_format( $val ) . " [" . MantisEnum::getLabel( $reproducibility_enum_string, $key ) . "]' />";
...
<div id="wrapper">
        ...
        <div id="filter">
            ...
            <form method="get">
                <input type="hidden" name="page" value="MantisStats/issues_by_reproducibility" />
