...
$query = "
        (
            SELECT mbt.id, COUNT( DISTINCT new_value ) AS the_count, mbt.status
            ...
            UNION
        (
            SELECT mbt.id, 0 AS the_count, mbt.status
            FROM $mantis_bug_table mbt
            LEFT JOIN $mantis_bug_history_table mbht 
            ON ( mbt.id = mbht.bug_id and mbht.field_name =  'handler_id' and new_value != '0' )
            WHERE $specific_where
            AND mbht.id is null
            AND date_submitted >= " . db_prepare_string( $db_datetimes['start'] ) . "
            AND date_submitted <= " . db_prepare_string( $db_datetimes['finish'] ) . "
...
$data_table_print['open'] = "
    <table class='display' id='open' style='display:none'>
        ...
        <tr class='tblheader'>
            ...
            <td class='dt-right nowrap'>" . lang_get( 'plugin_MantisStats_no_of_handlers' ) . "</td>
        </tr>
        </thead>
        <tbody>";
...
$nohandler['open'] = $nohandler['resolved'] = 0;
...
foreach ( $result as $row ) {
    ...
    if ( $row['status'] < $resolved_status_threshold ) {
        ...
        <tr>
            ...
            <td class='dt-right'>" . $extra[$row['id']]['nt'] . "</td>
        ...
        if( $row['the_count'] == 0 ) { $nohandler['resolved']++; }

        if ( ( VS_PRIVATE == bug_get_field( $row['id'], 'view_state' ) ) && ( false == access_has_bug_level( $private_bug_threshold, $row['id'] ) ) ) { continue; }
        if ( $i['resolved'] > $maxResultsInTables and $maxResultsInTables != 0 ) { continue; }
...
$nohandler['all_states'] = $nohandler['open'] + $nohandler['resolved'];
    ...
    <table class='display' id='summary' style='display:none'>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class='dt-right'>" . number_format( $nohandler['open'] ) . "</td>
                <td class='dt-right'>" . number_format( $nohandler['resolved'] ) . "</td>
                <td class='dt-right'>" . number_format( $nohandler['all_states'] ) . "</td>
            </tr>
        </tbody>
    </table>
    ";
