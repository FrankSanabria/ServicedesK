...
// This function to be removed in the future, see http://www.mantisbt.org/bugs/view.php?id=17073
...
$reports_arr = array(
    'issues_by_projects'            => lang_get( 'plugin_MantisStats_by_project' ),
    'issues_by_status'              => lang_get( 'plugin_MantisStats_by_status' ),
    'issues_by_severity'            => lang_get( 'plugin_MantisStats_by_severity' ),
    'issues_by_priority'            => lang_get( 'plugin_MantisStats_by_priority' ),
    'issues_by_category'            => lang_get( 'plugin_MantisStats_by_category' ),
    'issues_by_reproducibility'     => lang_get( 'plugin_MantisStats_by_reprod' ),
    'issues_by_resolution'          => lang_get( 'plugin_MantisStats_by_resol' ),
    'issues_by_handlers'            => lang_get( 'plugin_MantisStats_handler_stats' ),
    'issues_by_monitors'            => lang_get( 'plugin_MantisStats_monitoring_stats' ),
    'issues_by_notes'               => lang_get( 'plugin_MantisStats_notes_stats' ),
    'issues_by_tags'                => lang_get( 'plugin_MantisStats_tag_stats' ),
    'issues_by_custom_fields'       => lang_get( 'plugin_MantisStats_by_custom_fields'),
    'issues_by_reopenings'          => lang_get( 'plugin_MantisStats_by_reopenings'),           // PRO report
    'issues_by_attachments'         => lang_get( 'plugin_MantisStats_by_attachments'),          // PRO report
    'people_by_reporters'           => lang_get( 'plugin_MantisStats_reporters' ),
    'people_by_handlers'            => lang_get( 'plugin_MantisStats_handlers' ),
    'people_by_monitors'            => lang_get( 'plugin_MantisStats_monitors' ),
    'people_by_notes'               => lang_get( 'plugin_MantisStats_people_notes' ),
    'time_to_resolution'            => lang_get( 'plugin_MantisStats_by_time_to_resolution'),   // PRO report
    'time_in_state'                 => lang_get( 'plugin_MantisStats_by_time_in_state'),        // PRO report
    'time_to_first_note'            => lang_get( 'plugin_MantisStats_by_time_to_first_note'),   // PRO report
    'trends_by_open_resolved'       => lang_get( 'plugin_MantisStats_trends_open_res' ),
    'trends_by_notes'               => lang_get( 'plugin_MantisStats_trends_notes' )
);
...
if ( isset( $_REQUEST['page'] ) and !empty( $_REQUEST['page'] ) ) {
    ...
    $current_page = substr( $current_page, strlen( lang_get( 'plugin_MantisStats_title' ) ) + 1 );
}

if ( !in_array( $current_page, $reportsToShow ) and !in_array( $current_page, array( 'start_page', 'config_admin', 'config_admin_update' ) ) ) {
    echo '<center>';
    echo '<p>' . error_string( ERROR_ACCESS_DENIED ) . '</p>';
    print_bracket_link( plugin_page( 'start_page' ), lang_get( 'proceed' ) );
    echo '</center>';
    exit;
}

$whichReport = "<select id='basicFancy' onchange='this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);'>\n";

foreach ( $reportsToShow as $key => $val ) {
    if ( $val == $current_page ) { $selected = ' selected '; } else { $selected = ''; }
    $whichReport .= "<option " . $selected . " value='./plugin.php?page=MantisStats/" . $val . "'>" . $reports_arr[$val] . "</option>\n";
}
...
$startDateInputFilter_arr = array (
    1 => lang_get( 'plugin_MantisStats_start_date_option1' ),
    2 => lang_get( 'plugin_MantisStats_start_date_option2' ),
    3 => lang_get( 'plugin_MantisStats_start_date_option3' ),
    4 => lang_get( 'plugin_MantisStats_start_date_option4' ),
    5 => lang_get( 'plugin_MantisStats_start_date_option5' ),
);
...
$dt_language_snippet = '
            ...
            "language": {
                "processing":     "' . lang_get( 'plugin_MantisStats_dt_processing' ) . '",
                "info":           "' . lang_get( 'plugin_MantisStats_dt_info' ) . '",
                "infoEmpty":      "' . lang_get( 'plugin_MantisStats_dt_infoEmpty' ) . '",
                "infoPostFix":    "' . lang_get( 'plugin_MantisStats_dt_infoPostFix' ) . '",
                "loadingRecords": "' . lang_get( 'plugin_MantisStats_dt_loadingRecords' ) . '",
                "zeroRecords":    "' . lang_get( 'plugin_MantisStats_dt_zeroRecords' ) . '",
                "emptyTable":     "' . lang_get( 'plugin_MantisStats_dt_emptyTable' ) . '",
                "paginate": {
                    "first":      "' . lang_get( 'plugin_MantisStats_dt_first' ) . '",
                    "previous":   "' . lang_get( 'plugin_MantisStats_dt_previous' ) . '",
                    "next":       "' . lang_get( 'plugin_MantisStats_dt_next' ) . '",
                    "last":       "' . lang_get( 'plugin_MantisStats_dt_last' ) . '"
                },
                "aria": {
                    "sortAscending":  "' . lang_get( 'plugin_MantisStats_dt_sortAscending' ) . '",
                    "sortDescending": "' . lang_get( 'plugin_MantisStats_dt_sortDescending' ) . '"
                }
            }
