...
function tables_and_charts ( $type, $output ) {
	global $project_names, $status_values, $resolved_status_threshold, $status_enum_string, $issues_fetch_from_db, $maxResultsInTables;
    ...
    <table class='display' id='$type' style='display:none'>
        <thead>
        <tr class='tblheader'>
            <td width='100%'>" . lang_get( 'plugin_MantisStats_project_name' ) . "</td>";
...
EOT;
    ...
    foreach ( $data_table_totals as $key => $val ) {
        ...
        $chart_data_print .= "<set label='" . htmlspecialchars( $project_names[$key], ENT_QUOTES ) . "' value='" . $val . "' toolText='" . number_format( $val ) . " [" . htmlspecialchars( $project_names[$key], ENT_QUOTES ) . "]' />";
