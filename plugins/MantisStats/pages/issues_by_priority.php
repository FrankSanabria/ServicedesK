...
$query = "
        SELECT count(*) as the_count, priority, status
...
foreach ( $result as $row ) {
    $issues_fetch_from_db[$row['priority']][$row['status']] = $row['the_count'];
}
...
function tables_and_charts ( $type, $output ) {
	...
	global $resolved_status_threshold, $status_values, $status_enum_string, $priority_enum_string, $issues_fetch_from_db, $maxResultsInTables;
    ...
    <table class='display' id='$type' style='display:none'>
        <thead>
        <tr class='tblheader'>
            <td width='100%'>" . lang_get( 'priority' ) . "</td>";
    ...
    foreach ( $data_table_totals as $key => $val ) {
		...
		$data_table_print .= "
        <tr>
            <td>" . MantisEnum::getLabel( $priority_enum_string, $key ) . "</td>";
...
EOT;
    ...
    foreach ( $data_table_totals as $key => $val ) {
        ...
        $chart_data_print .= "<set label='" . MantisEnum::getLabel( $priority_enum_string, $key ) . "' value='" . $val . "' toolText='" . number_format( $val ) . " [" . MantisEnum::getLabel( $priority_enum_string, $key ) . "]' />";
...
<div id="wrapper">
        ...
        <div id="filter">
            ...
            <form method="get">
                <input type="hidden" name="page" value="MantisStats/issues_by_priority" />
