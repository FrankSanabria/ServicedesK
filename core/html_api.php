...
//		echo is_blank( $t_realname ) ? "($t_access_level)" : "" . string_html_specialchars( $t_realname );
//		echo '<a style="font-weight:bold" href="' . helper_mantis_url( 'account_page.php">' ) . string_html_specialchars( $t_realname ) . '</a>';
//		echo " - $t_access_level";
//		echo "</span>";
	}
}
...
//	event_signal( 'EVENT_LAYOUT_PAGE_FOOTER' );
	...
	if ( !is_page_name( 'login_page' ) ) {
		...
		echo '<a href="http://www.sugeek.co/servicedesk/" title="Su|GE3K ServiceDesk"><img src="' . helper_mantis_url( 'images/LogoService.png' ) . '" width="145" alt="Powered by SuGE3K Service Desk" border="0" /></a>';
		echo '</div>', "\n";
	}
...
function print_menu() {
	if( auth_is_user_authenticated() ) {
		...
		echo '<tr>';
		echo '<td class="menu" style="padding: 10px 0px 10px 0px;position:fixed; top:0; left:0;z-index:1000" >';
...
# Project Selector hidden if only one project visisble to user
	...
	if( OFF != config_get( 'rss_enabled' ) ) {
		...
		# My View
		$t_menu_options[] = '<a href="' . helper_mantis_url( 'my_view_page.php') . '" title="' . lang_get('my_view_link') . '"><i class="material-icons">dashboard</i></a>';

		# View Bugs
		$t_menu_options[] = '<a href="' . helper_mantis_url( 'view_all_bug_page.php') . '" title="' . lang_get( 'view_bugs_link' ) . '"><i class="material-icons">receipt</i></a>';

		# Report Bugs
		if( access_has_project_level( config_get( 'report_bug_threshold' ) ) ) {
			$t_menu_options[] = '<a href="' . helper_mantis_url( 'bug_report_page.php')  . '" title="' . lang_get('report_bug_link') . '"><i class="material-icons">mode_edit</i></a>';
		}
		# Changelog Page
		...
		$t_menu_options[] = '<a href="' . helper_mantis_url( 'summary_page.php' ) . '"title="' . lang_get( 'summary_link' ) .  '"><i class="material-icons">insert_chart</i></a>';
		}

		# Project Documentation Page
		if( ON == config_get( 'enable_project_documentation' ) ) {
		$t_menu_options[] = '<a href="' . helper_mantis_url( 'proj_doc_page.php' ) . '"title="' .  lang_get( 'docs_link' ) . '"><i class="material-icons">insert_drive_file</i></a>';
		}
		...
		if( access_has_global_level( config_get( 'manage_site_threshold' ) ) ) {
			...
			$t_menu_options[] = '<a href="'. $t_link . '" title="' . lang_get( 'manage_link' ) . '"><i class="material-icons">settings</i></a>';
		} else {
		...
		if( OFF == $t_protected ) {
			$t_menu_options[] = '<a href="' . helper_mantis_url( 'account_page.php' ) . '" title="' . lang_get( 'account_link' ) . '"><i class="material-icons">person</i></a>';
		}
		...
		if( config_get( 'time_tracking_enabled' ) && access_has_global_level( config_get( 'time_tracking_reporting_threshold' ) ) ) {
			$t_menu_options[] = '<a href="' . helper_mantis_url( 'billing_page.php' ) . '" title="' . lang_get( 'time_tracking_billing_link' ) . '"><i class="material-icons">timer</i></a>';
		...
		if( !current_user_is_anonymous() ) {
		$t_menu_options[] = '<a href="' . helper_mantis_url( 'logout_page.php' ) . '"title="' . lang_get( 'logout_link' ) . '"><i class="material-icons" style="color:red">close</i></a>';
...
//		echo lang_get( 'email_project' ), ': ';
		...
		if( ALL_PROJECTS == helper_get_current_project() ) {
			...
			if( ON == config_get( 'use_javascript' ) ) {
				...
				echo "<input type=\"search\" name=\"bug_id\" size=\"10\" value=\"$t_bug_label\" onfocus=\"if (this.value == '$t_bug_label') this.value = ''\" onblur=\"if (this.value == '') this.value = '$t_bug_label'\" />";
			} else {
				echo "<input  type=\"search\" name=\"bug_id\" size=\"10\" />&#160;";
			}
...
function print_summary_submenu() {
	echo '<div align="center" class="summary_menu">';
...
function print_manage_menu( $p_page = '' ) {
	...
	echo '<div align="center" class="admin_menu"><p>';
	if( access_has_global_level( config_get( 'manage_user_threshold' ) ) ) {
...
function print_manage_config_menu( $p_page = '' ) {
	...
	echo '<br /><div align="center" class="admin_menu">';
	if( access_has_project_level( config_get( 'view_configuration_threshold' ) ) ) {
