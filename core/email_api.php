...
function email_signup( $p_user_id, $p_password, $p_confirm_hash, $p_admin_name = '' ) {
	...
	if( $p_admin_name ) {
        ...
        $t_message = '<div align="center" style="width:100%;padding:5px"><img src="' . $g_path . 'images/Logo.png" /> </div><br />';
	$t_message .= $intro_text . "<br /><br />" . string_get_confirm_hash_url( $p_user_id, $p_confirm_hash ) ;
	$t_message .= "<br /><br />" . lang_get( 'new_account_message' ) . "<br /><b>" . lang_get( 'new_account_do_not_reply' ) . "</b>";
...
function email_send_confirm_hash_url( $p_user_id, $p_confirm_hash ) {
	...
	$t_subject = '[' . config_get( 'window_title' ) . '] ' . lang_get( 'lost_password_subject' );
        $t_message = '<div align="center" style="width:100%;padding:5px"><img src="' . $g_path . 'images/Logo.png" /> </div><br />';
	$t_message .=  lang_get( 'reset_request_msg' ) . "<br /><br />" . string_get_confirm_hash_url( $p_user_id, $p_confirm_hash ) . "<br /><br />";
	$t_message .= lang_get( 'new_account_username' ) . ' ' . $t_username . "<br />" . lang_get( 'new_account_IP' ) . ' ' . $_SERVER["REMOTE_ADDR"];
	$t_message .= "<br /><br /><b>" . lang_get( 'new_account_do_not_reply' ) . '</b>';
...
#	$t_message = $t_email_separator1 . " \n";
	...
	if( isset( $p_visible_bug_data['email_bug_view_url'] ) ) {
                $t_message .= "<br /><b>Ingrese al Caso: </b>" . $p_visible_bug_data['email_bug_view_url'] . " <br />";
#		$t_message .= $t_email_separator1 . " \n";
		$t_message .= "<hr /><br />";
	}
...
#		$t_message .= $t_bugnote->note . " \n\n";
                $t_message .= '<hr style="border-style:dotted" />' . "<br />";
                $t_message .= user_get_name( $t_bugnote->reporter_id ) . " " . $t_last_modified . "<br />";
                $t_message .= $t_bugnote->note . " <br /><br />";
